    ...
    <PageShell locale={locale} current="home">
          ...
          <div className="w-full">
            ...
            <div className="metric-grid mt-14 border-y border-white/20">
              {copy.home.proof.map((item) => (
                <div key={item.label} className="border-white/20 py-6 pr-5 lg:border-r">
                  <strong className="block text-3xl font-semibold">{item.value}</strong>
                  <span className="mt-2 block text-sm leading-6 text-neutral-300">{item.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="section">
        <div className="section-inner grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div><p className="eyebrow">Positioning</p><h2 className="text-3xl font-semibold tracking-normal text-neutral-950 sm:text-4xl">{copy.home.introTitle}</h2></div>
          <div>
            <p className="text-lg leading-8 text-neutral-600">{copy.home.introBody}</p>
            <div className="mt-10 grid gap-4 sm:grid-cols-2">
              {copy.home.pillars.map((pillar) => <div key={pillar} className="border-l-2 border-blue-900 bg-neutral-50 px-5 py-4 text-sm font-semibold text-neutral-800">{pillar}</div>)}
            </div>
          </div>
        </div>
      </section>
